...
exports.createClient = function(port, host, user, pass, maxListeners, secure, urlContextPath) {
  if (isNaN(port)) {
    host = port;
    port = 5984;
  }
  host = host || "localhost";
  urlContextPath = urlContextPath || undefined;
  ...
  couchClient.__defineGetter__('urlContextPath', function() {
  ...
  couchClient._queueRequest = function(options, cb) {
    ...
    if (urlContextPath !== undefined) {
      reqOptions.path = urlContextPath + options.path;
    }

    var
      request = httpClient.request(reqOptions),
      cbFired = false,
...
Db.prototype.changes = function(query, cb) {
  ...
  if (query.data) {
    reqOptions.data = query.data;
    delete query.data;
  }

  return this.request(reqOptions, cb);
};
